 ...
 * This class is to create an ep album 
 * */
using System;
...
namespace KpopAlbumDatabase
{
    class EP: Album
    {
        public EP(string title, string artist, int year, string category, string description, Image albumImage, string albumType)
        {
            // create a single
            this.title = title;
            this.artist = artist;
        ...
        public override bool AddSongs(Songs song)
        {
            ...
            if (songs.Contains(song) || songs.Count == 7)
            {
                return false;
            }
        ...
        public override bool RemoveSongs(Songs song)
        {
            ...
            if (songs.Contains(song) && songs.Count > 4)
            {
                songs.Remove(song);
                return true;
            }
